...
class LongTermMemoryAppService
{
     ...
     * 获取用户的记忆列表.
     */
    public function getMemoryList(MemoryListDTO $dto): array
    {
        return match ($dto->filterType) {
            'type' => $this->longTermMemoryDomainService->findByType($dto->orgId, $dto->appId, $dto->userId, MemoryType::from($dto->filterValue)),
            'tags' => $this->longTermMemoryDomainService->findByTags($dto->orgId, $dto->appId, $dto->userId, explode(',', $dto->filterValue)),
            'search' => $this->longTermMemoryDomainService->searchByContent($dto->orgId, $dto->appId, $dto->userId, $dto->filterValue),
            default => $this->longTermMemoryDomainService->findByUser($dto->orgId, $dto->appId, $dto->userId),
        };
    }
    ...
    {
        $memories = $this->longTermMemoryDomainService->searchByContent($orgId, $appId, $userId, $keyword);
