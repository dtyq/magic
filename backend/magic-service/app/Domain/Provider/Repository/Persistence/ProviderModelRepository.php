...
class ProviderModelRepository extends AbstractProviderModelRepository implements ProviderModelRepositoryInterface
{
    ...
    public function getByConfigIdAndSourceModelId(
        ...
        $builder = $this->createBuilder($dataIsolation, ProviderModelModel::query());
        $builder->where('service_provider_config_id', $configId)
    ...
    public function getSyncedModelsByConfigId(
        ...
        $builder = $this->createBuilder($dataIsolation, ProviderModelModel::query());
        $builder->where('service_provider_config_id', $configId)
            ->whereNotNull('source_model_id');

        $result = Db::select($builder->toSql(), $builder->getBindings());

        $syncedModels = [];
        foreach ($result as $row) {
            $entity = ProviderModelAssembler::toEntity($row);
            $syncedModels[$row['source_model_id']] = $entity;
        }
    ...
    public function deleteByConfigIdExceptSourceModelIds(
        ...
        if (! empty($keepSourceModelIds)) {
            $query->whereNotIn('source_model_id', $keepSourceModelIds);
        }
    ...
    public function update(
        ProviderDataIsolation $dataIsolation,
        ProviderModelEntity $modelEntity
    ): ProviderModelEntity {
        $attributes = $this->entityToAttributes($modelEntity);
    ...
    public function createOrUpdate(
        ...
        if ($modelEntity->getId()) {
            return $this->update($dataIsolation, $modelEntity);
        }
        return $this->create($dataIsolation, $modelEntity);
    }
    ...
    public function getByProviderConfigIds(
        ...
        $builder = $this->createBuilder($dataIsolation, ProviderModelModel::query());
        $builder->whereIn('service_provider_config_id', $configIds)
            ->where('status', Status::Enabled->value);

        $result = Db::select($builder->toSql(), $builder->getBindings());

        return array_map(
            fn ($row) => ProviderModelAssembler::toEntity($row),
            $result
        );
    }
     ...
     * 将Entity转换为数组属性.
     */
    private function entityToAttributes(ProviderModelEntity $entity): array
    {
        return [
            'id' => $entity->getId(),
            'service_provider_config_id' => $entity->getServiceProviderConfigId(),
            'source_model_id' => $entity->getSourceModelId(),
            'name' => $entity->getName(),
            'model_version' => $entity->getModelVersion(),
            'category' => $entity->getCategory()->value,
            'model_id' => $entity->getModelId(),
            'model_type' => $entity->getModelType()->value,
            'config' => $entity->getConfig() ? $entity->getConfig()->toArray() : [],
            'description' => $entity->getDescription(),
            'sort' => $entity->getSort(),
            'icon' => $entity->getIcon(),
            'organization_code' => $entity->getOrganizationCode(),
            'status' => $entity->getStatus()->value,
            'disabled_by' => $entity->getDisabledBy() ? $entity->getDisabledBy()->value : null,
            'translate' => $entity->getTranslate(),
            'model_parent_id' => $entity->getModelParentId(),
            'visible_organizations' => $entity->getVisibleOrganizations(),
            'visible_applications' => $entity->getVisibleApplications(),
            'visible_packages' => $entity->getVisiblePackages(),
            'load_balancing_weight' => $entity->getLoadBalancingWeight(),
            'is_office' => $entity->getIsOffice() ? 1 : 0,
            'super_magic_display_state' => $entity->getSuperMagicDisplayState(),
