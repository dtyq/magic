...
# source 过滤参数合法值
_VALID_SOURCES = {"all", "builtin", "workspace"}
...
class SkillListParams(BaseToolParams):
    ...
    source: str = Field(
        "all",
        description="""<!--zh: 来源过滤，可选值：all（全部）、builtin（内置）、workspace（用户安装和创建）-->
Source filter. Options: all (default), builtin (built-in only), workspace (user-installed and custom)""",
    )
...
class SkillList(BaseTool[SkillListParams]):
    """<!--zh
    列出当前所有可用 skill 的工具，包含内置 skill 和 workspace skill（skillhub 安装 + 用户创建）。
    每个 skill 标注来源和是否可被覆盖（内置 skill 优先级最高，不可覆盖）。
    在创建新 skill 前，建议先调用此工具检查是否存在同名 skill。
    -->
    Tool that lists all available skills, including built-in skills and workspace skills (skillhub-installed and custom).
    Each skill shows its source and whether it can be overridden (built-in skills have highest priority and cannot be overridden).
    ...
    async def execute(self, tool_context: ToolContext, params: SkillListParams) -> ToolResult:
        ...
        if source_filter in ("all", "workspace"):
            ...
            # 过滤掉与内置 skill 同名的 workspace skill（内置优先，workspace 版本实际不可加载）
            builtin_names = {s["name"] for s in skills}
            for ws in workspace:
                if ws["name"] in builtin_names:
                    ws["note"] = "shadowed by built-in skill with same name (not loadable)"
                skills.append(ws)
