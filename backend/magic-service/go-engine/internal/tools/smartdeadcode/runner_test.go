...
import (
	...
	smartdeadcode "magic/internal/tools/smartdeadcode"
)

var errTransientDeadcodeFailure = errors.New("temporary deadcode execution failure")
...
func TestRunnerSkipsWhenInputsUnchanged(t *testing.T) {
	...
	assertRunCount(t, runCount, 1)
	if !strings.Contains(output.String(), "Skipping deadcode (no changes since last run)") {
	...
	assertRunCount(t, runCount, 1)
	if !strings.Contains(output.String(), "Skipping deadcode (content unchanged)") {
...
func TestRunnerCachesFailedResult(t *testing.T) {
	...
	runner := newTestRunner(t, rootDir, cacheFile, &output, func(context.Context, string) (string, error) {
		runCount++
		return "deadcode finding from cache test", nil
	})
	...
	assertRunCount(t, runCount, 1)
	if !strings.Contains(output.String(), "deadcode finding from cache test") {
	...
	assertRunCount(t, runCount, 1)
	if !strings.Contains(output.String(), "Skipping deadcode (no changes since last run; cached failed result)") {
...
func TestRunnerDoesNotCacheExecutionFailure(t *testing.T) {
	...
	runner := newTestRunner(t, rootDir, cacheFile, &output, func(context.Context, string) (string, error) {
		runCount++
		if runCount == 1 {
			return errTransientDeadcodeFailure.Error(), errTransientDeadcodeFailure
		}
		return "", nil
	})
	...
	assertRunCount(t, runCount, 1)
	if !strings.Contains(output.String(), errTransientDeadcodeFailure.Error()) {
...
func newTestRunner(t *testing.T, rootDir, cacheFile string, output *bytes.Buffer, run func(context.Context, string) (string, error)) *smartdeadcode.Runner {
	...
	smartdeadcode.SetFileHashForTest(runner, func(string) (string, error) {
		return "deadcode-binary-hash", nil
	})
	smartdeadcode.SetRunDeadcodeForTest(runner, run)
