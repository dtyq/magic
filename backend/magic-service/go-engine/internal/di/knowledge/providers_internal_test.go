...
func TestDocumentResyncRabbitMQSchedulerConfigDefaultConsumerConcurrency(t *testing.T) {
	...
	defaults := documentsync.DefaultRabbitMQSchedulerConfig()
	if defaults.ConsumerConcurrency != 2 {
		t.Fatalf("expected default consumer concurrency 2, got %d", defaults.ConsumerConcurrency)
	}

	cfg := &autoloadcfg.Config{}
	got := newDocumentResyncRabbitMQSchedulerConfig(cfg, defaults)
	if got.ConsumerConcurrency != 2 {
		t.Fatalf("expected empty config to use default consumer concurrency 2, got %d", got.ConsumerConcurrency)
	}

	cfg.RabbitMQ.DocumentResync.ConsumerConcurrency = 4
	got = newDocumentResyncRabbitMQSchedulerConfig(cfg, defaults)
	if got.ConsumerConcurrency != 4 {
		t.Fatalf("expected configured consumer concurrency 4, got %d", got.ConsumerConcurrency)
	}
}
