...
**合并优先原则**：拆分前先判断——如果某个 reference 是 Skill 每次触发都必须读取的内容（如人格定义、语气规则、核心示例），应合并到 SKILL.md 中。reference/ 只放"按需查阅"的内容，即大部分请求不需要、仅特定场景才深入了解的内容。多一次文件读取 = 多一次 LLM 请求，在可以合并的情况下不要拆分。
-->
