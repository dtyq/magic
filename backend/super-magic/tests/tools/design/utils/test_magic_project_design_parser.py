...
async def test_video_element_round_trip(tmp_path):
    """视频元素应能稳定写入并读回 magic.project.js。"""
        ...
        canvas=CanvasConfig(
            elements=[
                VideoElement(
                    ...
                    width=1920,
                    height=1080,
                    src="demo/videos/sample.mp4",
                    poster="demo/videos/sample_poster.jpg",
                    status="completed",
                    generateVideoRequest={
                        ...
                        "operation_id": "op_123",
                        "aspect_ratio": "16:9",
                        "reference_images": ["demo/assets/ref-1.png"],
                        "frames": [
                            {"role": "start", "uri": "demo/assets/frame-start.png"},
                            {"role": "end", "uri": "demo/assets/frame-end.png"},
                        ],
                        "file_dir": "demo/videos",
                    },
                )
            ]
        ),
    ...
    with patch("app.tools.design.utils.magic_project_design_parser.PathManager.get_workspace_dir", return_value=tmp_path):
        await write_magic_project_js("demo", config)
        parsed = await read_magic_project_js("demo")

    assert parsed.canvas is not None
    assert len(parsed.canvas.elements) == 1
    element = parsed.canvas.elements[0]
    assert isinstance(element, VideoElement)
    assert element.type == "video"
    assert element.src == "demo/videos/sample.mp4"
    assert element.poster == "demo/videos/sample_poster.jpg"
    assert element.generateVideoRequest["operation_id"] == "op_123"
    assert element.generateVideoRequest["reference_images"] == ["demo/assets/ref-1.png"]
...
def test_validate_project_config_accepts_video_element():
    """video 元素应被视为合法类型。"""
...
class TestElementParsingFallback:
    ...
    def test_parse_image_element_missing_dimensions(self):
        ...
        assert element.y == 50
        assert element.width == DEFAULT_ELEMENT_WIDTH
        assert element.height == DEFAULT_ELEMENT_HEIGHT
...
"""
        ...
        # 现在 frame 是合法类型，应保留 4 个元素
        assert len(config.canvas.elements) == 4
        ...
        assert elem2.x == 50
        assert elem2.y == 50
        assert elem2.width == DEFAULT_ELEMENT_WIDTH
        assert elem2.height == DEFAULT_ELEMENT_HEIGHT
        ...
        # 验证第四个元素（frame，字段自动填充）
        elem4 = config.canvas.elements[3]
        assert elem4.id == "elem-4"
        assert elem4.type == "frame"
        assert elem4.x == 0.0
        assert elem4.y == 0.0
        assert elem4.width == DEFAULT_ELEMENT_WIDTH
        assert elem4.height == DEFAULT_ELEMENT_HEIGHT
