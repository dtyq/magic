...
import { revokeImageUrl } from "@/opensource/services/tiptap-image-storage"
...
import {
	...
	ResizeHandles,
	ObjectFitToolbar,
} from "@/opensource/components/tiptap-node/project-image-node/components"
import {
	useIntersectionObserver,
	useImageResize,
} from "@/opensource/components/tiptap-node/project-image-node/hooks"
import "@/opensource/components/tiptap-node/storage-image-node/storage-image-node.scss"
...
export function StorageImageNodeView(props: StorageImageNodeViewProps) {
	...
	return (
		<NodeViewWrapper
			ref={containerRef}
			className={`storage-image-node ${selected ? "storage-image-node--selected" : ""} ${
				isResizing ? "storage-image-node--resizing" : ""
			} ${alignClass}`}
		>
