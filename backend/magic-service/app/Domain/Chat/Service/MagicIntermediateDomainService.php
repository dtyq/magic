...
class MagicIntermediateDomainService extends AbstractDomainService
{
    ...
    public function handleSuperMagicInstructionMessage(
        ...
        try {
            ...
            // 3. 创建序列实体 (临时消息不需要持久化序列)
            $seqEntity = new MagicSeqEntity();
            $seqEntity->setAppMessageId($messageDTO->getAppMessageId());
            $seqEntity->setConversationId($conversationEntity->getId()); // 设置会话ID
            // 注意：MagicSeqEntity可能没有setSenderId/setReceiveId方法，使用实际存在的方法
            $seqEntity->setObjectId($senderUserId); // 设置对象ID为发送者
            $seqEntity->setContent($messageDTO->getContent()); // 使用消息内容
            ...
            if (! empty($topicId)) {
                $this->validateTopicOwnership($topicId, $conversationEntity->getId(), $dataIsolation);
            }

            $seqExtra->setTopicId($topicId);
            $seqEntity->setExtra($seqExtra);

            // 4. 创建消息实体 (转换DTO为Entity，但不持久化)
