...
需要生成图片？
├─ 有参考图？→ 先查询尺寸，再调用 generate_canvas_images
├─ 不同主题？→ 使用多个 prompts
...
result = tool.call('visual_understanding', {
    "images": ["my-design/images/cat.jpg"],
    "question": "Describe this image in detail, including its content and dimensions."
})

# Step 2: 基于理解结果生成新图（size 省略时自动从参考图读取）
if result.ok:
    result2 = tool.call('generate_canvas_images', {
...
if result.ok and result.data:
    # 直接访问字段
    created = result.data.get('created_elements', [])
    if created:
...
**在拆分之前，先判断 reference 文件是否为 Skill 运行时的必读内容。** 如果某个 reference 是 Skill 正常工作时每次都要加载的（如人格定义、语气规则、核心示例），应合并到 SKILL.md 中，而不是拆分到 reference/。

拆分到 reference/ 的内容应该是"按需查阅"的——即大部分请求不需要读取，只有特定场景才需要深入了解的内容（如高级配置、故障排查、完整 API 参考等）。

**判断标准：**
- 每次触发 Skill 都需要读取 → 合并到 SKILL.md（即使文件稍长也优于多一次工具调用）
- 仅特定场景需要查阅 → 放入 reference/
- 拆分后如果 SKILL.md 频繁指示"先读 reference/X 再回答" → 说明拆错了，应该合并回来
