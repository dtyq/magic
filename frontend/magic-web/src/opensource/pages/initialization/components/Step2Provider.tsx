...
import {
	...
	FormLabel,
	FormMessage,
} from "@/opensource/components/shadcn-ui/form"
...
export default function Step2Provider({
	...
	const handleTestConnection = async () => {
		...
		if (!values.service_provider_config?.url || !values.service_provider_config?.api_key) {
			setTestResult({
				success: false,
				message: t("step2.errors.missingConfig"),
			})
			return
		}
				...
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
					...
					<div className="space-y-6 rounded-lg bg-muted/50 p-6">
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									<FormLabel>{t("step2.deploymentName")}</FormLabel>
									<FormControl>
										<Input
