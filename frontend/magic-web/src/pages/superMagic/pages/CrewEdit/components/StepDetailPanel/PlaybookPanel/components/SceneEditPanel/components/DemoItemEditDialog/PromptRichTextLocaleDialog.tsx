...
interface PromptRichTextLocaleDialogProps {
	value: LocaleText
...
export function PromptRichTextLocaleDialog({
	value,
	onChange,
	placeholder,
	localizeLabel,
	...
	const editorRefs = useRef<Record<string, PromptRichTextEditorHandle | null>>({})
	...
	const isDefaultMissing = isPromptRichTextEmpty(draft[DEFAULT_LOCALE_KEY])
	...
	function setEditorRef(locale: string, editor: PromptRichTextEditorHandle | null) {
					...
					{SUPPORTED_LOCALES.map((localeKey) => (
						<div key={localeKey}>
							{renderLocaleField({
								...
								placeholder: !isPromptRichTextEmpty(draft[DEFAULT_LOCALE_KEY])
									? t("playbook.edit.basicInfo.localeDialog.usingDefault", {
											value: getPromptRichTextPlainText(
												draft[DEFAULT_LOCALE_KEY],
											),
										})
									: placeholder,
