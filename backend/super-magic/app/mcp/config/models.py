...
class MCPServerConfig(BaseModel):
    ...
    支持 HTTP/SSE 和 Stdio 两种连接类型，提供字段校验。
    """
    ...
    @classmethod
    def convert_env_list_to_dict(cls, v):
        """将空列表转换为空字典以兼容旧配置格式"""
        if isinstance(v, list):
            if len(v) == 0:
    ...
    def validate_config(self) -> None:
        ...
        if self.type == MCPServerType.HTTP:
            ...
            logger.debug(f"HTTP 服务器 '{self.name}' 配置验证通过")
    ...
    def __str__(self) -> str:
        if self.type == MCPServerType.HTTP:
            return f"MCPServer(name='{self.name}', type=HTTP)"
