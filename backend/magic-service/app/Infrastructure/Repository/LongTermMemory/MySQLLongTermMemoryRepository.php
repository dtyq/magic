...
class MySQLLongTermMemoryRepository implements LongTermMemoryRepositoryInterface
{
    ...
    public function findMemories(MemoryQueryDTO $dto): array
    {
        ...
        // 游标分页
        if ($dto->lastOrderValue !== null && $dto->lastId !== null) {
            $orderColumn = $dto->orderBy;
            $operator = $dto->orderDirection === 'desc' ? '<' : '>';
            $equalOperator = $dto->orderDirection === 'desc' ? '<' : '>';

            $query->where(function (Builder $subQuery) use ($orderColumn, $operator, $equalOperator, $dto) {
                $subQuery->where($orderColumn, $operator, $dto->lastOrderValue)
                    ->orWhere(function (Builder $nestedQuery) use ($orderColumn, $equalOperator, $dto) {
                        $nestedQuery->where($orderColumn, $dto->lastOrderValue)
                            ->where('id', $equalOperator, $dto->lastId);
                    });
            });
        }
