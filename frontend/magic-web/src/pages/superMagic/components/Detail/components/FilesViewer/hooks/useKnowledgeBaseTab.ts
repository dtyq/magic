...
function buildKnowledgeBaseTabId(data: KnowledgeBaseTabData) {
    return `${KNOWLEDGE_BASE_TAB_ID_PREFIX}${data.knowledgeBaseId}_${data.fileKey}`
}
    ...
    const openKnowledgeBaseTab = useCallback((data: KnowledgeBaseTabData) => {
        ...
        setKnowledgeBaseTabs((prev) => {
            ...
            const newTab: KnowledgeBaseTabItem = {
                ...
                active: true,
                closeable: true,
                fileData: (data.fileExtension ? { file_extension: data.fileExtension } : {}) as Record<string, never>,
                data,
                create_at: Date.now(),
            }
