...
<link
  href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,600;1,700&family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@400;500&family=Noto+Serif+SC:wght@600;700&family=Noto+Sans+SC:wght@400;500&display=swap"
  rel="stylesheet"
/>
```
...
option = {
  ...
  yAxis: {
    type: "value",
    axisLabel: { color: "#8A8A8A", fontSize: 13 },
    splitLine: { lineStyle: { color: "#EEEBE6" } },
    axisLine: { show: false },
  },
  series: [
    {
...
- **Masthead strip (`ed-mast-strip`)**: a three-part issue header for covers and section openers, using strict mono labels and restrained editorial red.
- **Issue index (`ed-index-list`, `ed-index-row`)**: a premium contents component with section number, headline, and page number, designed for high-density editorial navigation.
- **Sidebar note (`ed-sidebar-note`)**: serif italic commentary block for editor notes, expert quotes, or contextual analysis.
- **Caption stack (`ed-caption-stack`)**: photo placeholder plus source metadata; use for image-heavy city, interview, and culture pages.
- **Proof strip (`ed-proof-strip`)**: a narrow data evidence band for rankings, growth, and issue-level proof points.
