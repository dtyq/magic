...
function MagicDatePicker({ className, ...props }: MagicDatePickerProps): ReactElement {
	const { styles, cx } = useStyles()
	return <DatePicker className={cx(styles.datePicker, className)} {...props} />
}

function MagicDatePickerRangePicker({
	className,
	...props
}: MagicDatePickerRangePickerProps): ReactElement {
	const { styles, cx } = useStyles()
	return (
