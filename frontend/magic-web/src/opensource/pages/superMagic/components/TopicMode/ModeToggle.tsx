...
import IconComponent from "@/opensource/pages/superMagic/components/IconViewComponent/index"
import { MagicIcon } from "@/opensource/components/base"
import pubsub, { PubSubEvents } from "@/opensource/utils/pubsub"
import BlackPurpleButton from "@/opensource/components/other/BlackPurpleButton"
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
...
import {
	...
	PopoverContent,
	PopoverTrigger,
} from "@/opensource/components/shadcn-ui/popover"
...
import { DrawerTitle } from "@/opensource/components/shadcn-ui/drawer"
...
function ModeToggle({
	...
	const renderSelectionIndicator = useMemoizedFn((isSelected: boolean, testId?: string) => {
		return (
			...
			>
				<div
					className={cn(
						"shadow-xs flex size-4 items-center justify-center rounded-[4px] border transition-colors",
						isSelected
	...
	const modeListContent = useMemo(() => {
		return (
			...
			>
				...
				>
					{modeList?.map((tab) =>
						allowChangeMode ? renderDropdownModeItem(tab) : renderStaticModeItem(tab),
					)}
				</div>
			</div>
		)
	...
	const currentModeItem = useMemo(() => {
		...
		return (
			<div
				className={cn(
					"shadow-xs flex shrink-0 cursor-pointer items-center gap-2 rounded-full border border-border bg-background outline-none hover:bg-sidebar/50 focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 dark:bg-sidebar dark:hover:bg-muted",
	...
	if (isMobile) {
		return (
			<div className="relative" data-testid="super-message-editor-mode-toggle-root">
				<div onClick={() => setOpen(true)}>{currentModeItem}</div>
				<MagicPopup
					visible={open}
					onClose={() => {
						setOpen(false)
