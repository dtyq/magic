...
class CardCompletenessValidator:
    ...
    职责：验证 data.js 中的卡片是否与 cards_plan.md 中的规划一致
    """

    async def validate(self, project_dir: Path) -> None:
        """验证卡片完成度，对比data.js与cards_plan.md的卡片内容

        验证逻辑：
        1. 如果cards_plan.md文件不存在，跳过验证
        2. 使用正则匹配提取卡片信息
        3. 比对规划卡片与已实现卡片
        ...
        """
        cards_plan_path = project_dir / "cards_plan.md"
        data_js_path = project_dir / "data.js"

        # 如果cards_plan.md文件不存在，跳过验证
        if not cards_plan_path.exists():
            logger.info("cards_plan.md文件不存在，跳过卡片完成度验证")
            return
        ...
        try:
            # 读取文件内容
            with open(cards_plan_path, 'r', encoding='utf-8') as f:
                plan_content = f.read()
            with open(data_js_path, 'r', encoding='utf-8') as f:
                data_js_content = f.read()

            # 简化的卡片解析
            planned_cards = self._extract_planned_cards(plan_content)
            implemented_card_ids = self._extract_implemented_cards(data_js_content)

            # 如果无法解析到规划卡片，跳过验证
            if not planned_cards:
                logger.info("cards_plan.md文件中未找到有效的卡片规划，跳过验证")
                return
            ...
            logger.info(f"卡片完成度验证通过：规划中 {len(planned_cards)} 个卡片，全部已实现")

            # 验证成功后删除cards_plan.md文件
            try:
                await safe_delete(cards_plan_path)
                logger.info("已删除cards_plan.md文件")
            except Exception as delete_error:
                logger.warning(f"删除cards_plan.md文件失败，但不影响验证结果: {delete_error}")
    ...
    def _extract_planned_cards(self, plan_content: str) -> List[Dict[str, str]]:
        """从cards_plan.md中提取规划中的卡片信息
        ...
        Args:
            plan_content: cards_plan.md文件内容

        Returns:
            List[Dict[str, str]]: 卡片列表，包含name、type、card_id(可选)、description(可选)
        """
        cards = []
        valid_types = ['metric', 'table', 'markdown', 'echarts']
