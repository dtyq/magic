...
export const preprocessMarkdown = (content: string) => {
	...
	if (processedContent.includes("<")) {
		processedContent = processedContent.replace(CUSTOM_TAG_PATTERN, (match, tagName) => {
			// 如果是标准 HTML 标签，不转义
	...
	// if (processedContent.includes("_")) {
	// 	const protectedFilePathTags: string[] = []
	// 	processedContent = processedContent.replace(FILE_PATH_TAG_PATTERN, (match) => {
	// 		const placeholder = `@@FILEPATHTAG${protectedFilePathTags.length}@@`
	// 		protectedFilePathTags.push(match)
	// 		return placeholder
	// 	})

	// 	processedContent = processedContent.replace(
	// 		UNDERSCORE_EMPHASIS_PATTERN,
	// 		(match, content) => {
	// 			// 检查是否在反引号内
	...
	// 	if (protectedFilePathTags.length > 0) {
	// 		processedContent = processedContent.replace(/@@FILEPATHTAG(\d+)@@/g, (_, index) => {
	// 			const tag = protectedFilePathTags[Number(index)]
	// 			return tag ?? ""
	// 		})
	// 	}
	// }
