...
负责处理 V1 流式响应的核心逻辑，包括：
- 拆分 reasoning / content 两条流
- 管理 BEFORE/AFTER_AGENT_THINK 事件
- 分别触发 reasoning 和 content 的 before/after reply 事件
"""
...
from .streaming_context import StreamProcessContext, StreamProcessResult, FinishReason
...
class StreamResponseHandler(StreamResponseHandlerBase):
    """
    V1 流式响应处理器。
