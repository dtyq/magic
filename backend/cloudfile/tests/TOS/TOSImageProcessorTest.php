 ...
 * @coversNothing
 */
class TOSImageProcessorTest extends TestCase
{
    private TOSImageProcessor $processor;

    protected function setUp(): void
    {
        $this->processor = new TOSImageProcessor();
    }

    public function testGetParameterName()
    {
        $this->assertEquals('x-tos-process', $this->processor->getParameterName());
    }
    ...
    public function testCompatibilityWithOSS()
    {
        // TOS 的参数格式应该与 OSS 基本一致
        $options = (new ImageProcessOptions())
            ->resize(['width' => 800, 'mode' => 'lfit'])
            ->quality(90)
            ->format('webp');
        ...
        // 验证格式与 OSS 一致（除了参数名不同）
        $this->assertEquals('image/resize,m_lfit,w_800/quality,q_90/format,webp', $result);
    }
}
