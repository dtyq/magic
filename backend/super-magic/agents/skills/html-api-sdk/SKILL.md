...
const cancel = window.Magic.llm.stream(
  ...
  (delta, done) => { text += delta; if (done) console.log("Done"); },
...
```javascript
// Plain text
const {topicId} = await window.Magic.project.createTopicAndSend("Analyze this", {model: "auto"});
...
const {topicId: t2} = await window.Magic.project.createTopicAndSend({
  ...
  content: [{type: "paragraph", content: [
    ...
    {type: "mention", attrs: {type: "project_file", data: {
      file_id: "skill_ref", file_name: "SKILL.md",
      file_path: ".magic/report_writer/SKILL.md", file_extension: "md"
    }}},
    {type: "text", text: "\n\n任务：生成报告"}
  ]}]
}, {model: "auto"});
