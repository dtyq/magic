...
var (
	...
	ErrDigitalEmployeeSourceTypeRequired = knowledgebasedomain.ErrDigitalEmployeeSourceTypeRequired
	// ErrAmbiguousFlowSourceType 表示 flow 缺失 source_type 时无法从 binding 唯一推断。
	ErrAmbiguousFlowSourceType = knowledgebasedomain.ErrAmbiguousFlowSourceType
)

const knowledgeBaseSyncModeCreate = "create"
...
type knowledgeBaseDomainService interface {
	PrepareForSave(ctx context.Context, kb *knowledgebasedomain.KnowledgeBase) error
	Save(ctx context.Context, kb *knowledgebasedomain.KnowledgeBase) error
	Update(ctx context.Context, kb *knowledgebasedomain.KnowledgeBase) error
	UpdateProgress(ctx context.Context, kb *knowledgebasedomain.KnowledgeBase) error
	...
	Destroy(ctx context.Context, kb *knowledgebasedomain.KnowledgeBase) error
	DeleteVectorData(ctx context.Context, kb *knowledgebasedomain.KnowledgeBase) error
...
type sourceBindingRepository interface {
	ReplaceBindings(ctx context.Context, knowledgeBaseCode string, bindings []sourcebindingdomain.Binding) ([]sourcebindingdomain.Binding, error)
	SaveBindings(ctx context.Context, knowledgeBaseCode string, bindings []sourcebindingdomain.Binding) ([]sourcebindingdomain.Binding, error)
	DeleteBindingsByKnowledgeBase(ctx context.Context, knowledgeBaseCode string) error
	ListBindingsByKnowledgeBase(ctx context.Context, knowledgeBaseCode string) ([]sourcebindingdomain.Binding, error)
	UpsertSourceItem(ctx context.Context, item sourcebindingdomain.SourceItem) (*sourcebindingdomain.SourceItem, error)
	ReplaceBindingItems(ctx context.Context, bindingID int64, items []sourcebindingdomain.BindingItem) error
}
...
type knowledgeBasePermissionReader interface {
	ListOperations(
		ctx context.Context,
		organizationCode string,
		userID string,
...
// KnowledgeBaseOwnerGrantPort 定义知识库 owner 授权端口。
type KnowledgeBaseOwnerGrantPort interface {
	GrantKnowledgeBaseOwner(
		ctx context.Context,
		organizationCode string,
	...
	DeleteKnowledgeBasePermissions(
		ctx context.Context,
		organizationCode string,
		currentUserID string,
		knowledgeBaseCode string,
...
type KnowledgeBaseAppService struct {
	...
	ownerGrantPort        KnowledgeBaseOwnerGrantPort
...
type knowledgeBasePermissionOperation string

const (
	knowledgeBasePermissionNone  knowledgeBasePermissionOperation = "none"
	knowledgeBasePermissionOwner knowledgeBasePermissionOperation = "owner"
	knowledgeBasePermissionAdmin knowledgeBasePermissionOperation = "admin"
	knowledgeBasePermissionRead  knowledgeBasePermissionOperation = "read"
	knowledgeBasePermissionEdit  knowledgeBasePermissionOperation = "edit"
)

const (
	knowledgeBaseUserOperationNone  = 0
	knowledgeBaseUserOperationOwner = 1
	knowledgeBaseUserOperationAdmin = 2
	knowledgeBaseUserOperationRead  = 3
	knowledgeBaseUserOperationEdit  = 4
)

func parseKnowledgeBasePermissionOperation(raw string) knowledgeBasePermissionOperation {
	switch strings.ToLower(strings.TrimSpace(raw)) {
	case string(knowledgeBasePermissionOwner):
		return knowledgeBasePermissionOwner
	case string(knowledgeBasePermissionAdmin):
		return knowledgeBasePermissionAdmin
	case string(knowledgeBasePermissionRead):
		return knowledgeBasePermissionRead
	case string(knowledgeBasePermissionEdit):
		return knowledgeBasePermissionEdit
	default:
		return knowledgeBasePermissionNone
	}
}

func (op knowledgeBasePermissionOperation) canRead() bool {
	return op == knowledgeBasePermissionOwner ||
		op == knowledgeBasePermissionAdmin ||
		op == knowledgeBasePermissionRead ||
		op == knowledgeBasePermissionEdit
}

func (op knowledgeBasePermissionOperation) canEdit() bool {
	return op == knowledgeBasePermissionOwner ||
		op == knowledgeBasePermissionAdmin ||
		op == knowledgeBasePermissionEdit
}

func (op knowledgeBasePermissionOperation) canDelete() bool {
	return op == knowledgeBasePermissionOwner ||
		op == knowledgeBasePermissionAdmin
}

func (op knowledgeBasePermissionOperation) toUserOperation() int {
	switch op {
	case knowledgeBasePermissionOwner:
		return knowledgeBaseUserOperationOwner
	case knowledgeBasePermissionAdmin:
		return knowledgeBaseUserOperationAdmin
	case knowledgeBasePermissionRead:
		return knowledgeBaseUserOperationRead
	case knowledgeBasePermissionEdit:
		return knowledgeBaseUserOperationEdit
	default:
		return knowledgeBaseUserOperationNone
	}
}

func (op knowledgeBasePermissionOperation) validateAction(action string) bool {
...
// SetOwnerGrantPort 注入知识库 owner 授权端口。
func (s *KnowledgeBaseAppService) SetOwnerGrantPort(port KnowledgeBaseOwnerGrantPort) {
	if s == nil {
		return
	}
	s.ownerGrantPort = port
}
...
func (s *KnowledgeBaseAppService) entityToDTO(e *knowledgebasedomain.KnowledgeBase) *kbdto.KnowledgeBaseDTO {
	return EntityToDTO(e)
}

func (s *KnowledgeBaseAppService) entityToDTOWithContext(
	ctx context.Context,
	e *knowledgebasedomain.KnowledgeBase,
	...
	return applyKnowledgeBaseBindingInfo(dto, agentCodes, knowledgeBaseTypeFromKnowledgeBase(e)), nil
}

func (s *KnowledgeBaseAppService) entityToDTOWithResolvedModel(e *knowledgebasedomain.KnowledgeBase, effectiveModel string) *kbdto.KnowledgeBaseDTO {
	dto := s.entityToDTO(e)
	if dto == nil {
		return nil
	}
...
func (s *KnowledgeBaseAppService) entityToDTOWithKnownBindings(
	...
	return applyKnowledgeBaseBindingInfo(
...
func (s *KnowledgeBaseAppService) ensureKnowledgeBaseActionAllowed(
	...
	operation, err := s.resolveKnowledgeBasePermissionOperation(ctx, organizationCode, userID, knowledgeBaseCode)
	if err != nil {
		return knowledgeBasePermissionNone, err
	}
	if !operation.validateAction(action) {
...
func (s *KnowledgeBaseAppService) resolveKnowledgeBasePermissionOperation(
	ctx context.Context,
	organizationCode string,
	userID string,
	knowledgeBaseCode string,
	...
	if s == nil || s.permissionReader == nil {
		return knowledgeBasePermissionNone, ErrKnowledgeBasePermissionReaderRequired
	}
	operations, err := s.permissionReader.ListOperations(ctx, organizationCode, userID, []string{knowledgeBaseCode})
	if err != nil {
		return knowledgeBasePermissionNone, fmt.Errorf("list knowledge base permissions: %w", err)
	}
	return parseKnowledgeBasePermissionOperation(operations[knowledgeBaseCode]), nil
}
...
func applyKnowledgeBaseUserOperation(
	...
	dto.UserOperation = operation.toUserOperation()
	return dto
}
...
func (s *KnowledgeBaseAppService) filterReadableKnowledgeBaseCodes(
	...
	if s == nil || s.permissionReader == nil {
		return nil, nil, ErrKnowledgeBasePermissionReaderRequired
	}

	rawOperations, err := s.permissionReader.ListOperations(ctx, organizationCode, userID, requestedCodes)
	if err != nil {
		return nil, nil, fmt.Errorf("list knowledge base permissions: %w", err)
	}

	operations := make(map[string]knowledgeBasePermissionOperation, len(rawOperations))
	for code, operation := range rawOperations {
		operations[code] = parseKnowledgeBasePermissionOperation(operation)
	}

	if len(requestedCodes) > 0 {
		filtered := make([]string, 0, len(requestedCodes))
		for _, code := range requestedCodes {
			if operations[code].canRead() {
				filtered = append(filtered, code)
			}
		}
		return filtered, operations, nil
	}
