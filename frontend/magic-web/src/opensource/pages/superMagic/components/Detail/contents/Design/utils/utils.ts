import { getFileContentById } from "@/opensource/pages/superMagic/utils/api"
...
import { IMAGE_EXTENSIONS } from "@/opensource/constants/file"
import type { LayerElement } from "@/opensource/components/CanvasDesign/canvas/types"
import { t } from "i18next"
import { AttachmentSource } from "@/opensource/pages/superMagic/components/TopicFilesButton/hooks/types"
...
export async function packAndDownloadFiles(
	imageFiles: FileItem[],
	downloadMode?: import("@/opensource/pages/superMagic/pages/Workspace/types").DownloadImageMode,
	zipFileName = "design-images.zip",
): Promise<{
	successCount: number
	results: Array<{ success: boolean; fileName: string; error?: unknown }>
}> {
	const { loadJSZip } = await import("@/opensource/lib/jszip")
	const { getTemporaryDownloadUrl } = await import("@/opensource/pages/superMagic/utils/api")
