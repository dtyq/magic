...
class KeepaliveRegistry:
    ...
    def notify_activity(self, source: str, occurred_at_ms: int | None = None) -> None:
        ...
        current_value = self._last_activity_at_ms_by_source.get(normalized_source, 0)
        self._last_activity_at_ms_by_source[normalized_source] = max(current_value, message_at_ms)
        self.keepalive_once(f"{normalized_source}.activity")
    ...
    def restore_activity_time(self, source: str, last_activity_at_ms: int) -> None:
        ...
        if not normalized_source or last_activity_at_ms <= 0:
            return
        current_value = self._last_activity_at_ms_by_source.get(normalized_source, 0)
        restored_value = max(current_value, last_activity_at_ms)
        self._last_activity_at_ms_by_source[normalized_source] = restored_value
        if self._is_within_window(restored_value):
            self._ensure_loop_running()
            logger.info(
                f"[KeepaliveRegistry] 已恢复历史活跃时间: source={normalized_source}, "
                f"last_activity_at_ms={restored_value}"
            )
    ...
    def reset_source(self, source: str) -> None:
        ...
        self._last_activity_at_ms_by_source.pop(normalized_source, None)
    ...
    def _ensure_loop_running(self) -> None:
        ...
        logger.info("[KeepaliveRegistry] 保活循环已启动")

    async def _run(self) -> None:
        try:
            while True:
                latest_at_ms = self._get_latest_activity_at_ms()
                if latest_at_ms <= 0:
                    logger.info("[KeepaliveRegistry] 无可用活跃时间，停止保活循环")
                    return
                if not self._is_within_window(latest_at_ms):
                    logger.info(
                        "[KeepaliveRegistry] 最新活跃时间已超出保活窗口，停止保活循环: "
                        f"last_activity_at_ms={latest_at_ms}"
                    )
                    return
                await asyncio.sleep(KEEPALIVE_INTERVAL_SECONDS)
                latest_at_ms = self._get_latest_activity_at_ms()
                if latest_at_ms <= 0 or not self._is_within_window(latest_at_ms):
                    logger.info("[KeepaliveRegistry] 活跃窗口已失效，停止保活循环")
                    return
                self.keepalive_once("activity-window")
        except asyncio.CancelledError:
            logger.debug("[KeepaliveRegistry] 保活循环已取消")
            raise
        finally:
            self._task = None

    def _get_latest_activity_at_ms(self) -> int:
        if not self._last_activity_at_ms_by_source:
            return 0
        return max(self._last_activity_at_ms_by_source.values(), default=0)

    def _is_within_window(self, activity_at_ms: int) -> bool:
        return now_ms() - activity_at_ms <= _KEEPALIVE_WINDOW_MS
