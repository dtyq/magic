...
func (s *FragmentAppService) knowledgeAccessService() *kbaccess.Service {
	...
	return kbaccess.NewService(
		s.permissionReader,
		nil,
		&fragmentExternalAccessReader{support: s},
		nil,
	)
}

func (s *FragmentAppService) authorizeKnowledgeBaseAction(
	ctx context.Context,
	...
	if err != nil {
		return fmt.Errorf("authorize fragment knowledge base access: %w", err)
	}
	if !result.Operation.ValidateAction(action) {
		return fmt.Errorf("%w: action=%s knowledge_base_code=%s", ErrFragmentPermissionDenied, action, knowledgeBaseCode)
	}
	return nil
}

func resolveFragmentAccessActor(ctx context.Context, organizationCode, userID string) kbaccess.Actor {
...
type fragmentExternalAccessReader struct {
	support *FragmentAppService
}

func (r *fragmentExternalAccessReader) ListOperations(
	ctx context.Context,
...
type fragmentKnowledgeAccessPort interface {
