...
readonly class AsrFileAppService
{
    ...
    public function handleAsrSummary(
        ...
        try {
            ...
            if (! $summaryRequest->hasFileId()) {
                ...
                // 3.3 前端直传文件场景：根据生成的标题重命名文件所在目录
                if (! empty($summaryRequest->generatedTitle)) {
                    $this->handleUploadedFileRename($taskStatus, $summaryRequest, $userId);
                }
            }
    ...
    /**
     * 处理上传文件的重命名逻辑（前端直传文件场景）.
     *
     * @param AsrTaskStatusDTO $taskStatus 任务状态
     * @param SummaryRequestDTO $summaryRequest 总结请求
     * @param string $userId 用户ID
     */
    private function handleUploadedFileRename(
        AsrTaskStatusDTO $taskStatus,
        SummaryRequestDTO $summaryRequest,
        string $userId
    ): void {
        try {
            $fileEntity = $this->taskFileDomainService->getById((int) $taskStatus->audioFileId);
            if ($fileEntity === null) {
                $this->logger->warning('上传文件不存在，跳过重命名', [
            ...
            $workspaceRelativePath = AsrAssembler::extractWorkspaceRelativePath($fileEntity->getFileKey());
            $directoryPath = dirname($workspaceRelativePath);
            $fileName = basename($workspaceRelativePath);

            // 检查文件是否在根目录，如果是则跳过（不重命名根目录）
            if ($directoryPath === '.' || empty($directoryPath)) {
                $this->logger->info('文件在根目录，跳过目录重命名', [
                    'file_path' => $workspaceRelativePath,
                    'task_key' => $taskStatus->taskKey,
                ]);
                return;
            ...
            // 检查目录是否需要重命名（如果目录名是时间戳格式或默认名称）
            if ($this->shouldRenameDirectory($directoryName)) {
                $this->renameUploadedFileDirectory(
                    $fileEntity,
                    $directoryPath,
                    $directoryName,
                    $fileName,
                ...
                $this->logger->info('目录名称已有意义，跳过重命名', [
                    'directory_name' => $directoryName,
                    'task_key' => $taskStatus->taskKey,
                ]);
            }
        } catch (Throwable $e) {
            $this->logger->error('处理上传文件重命名失败', [
                'task_key' => $taskStatus->taskKey,
                'error' => $e->getMessage(),
            ]);
            // 不抛出异常，避免影响主流程
     ...
     * @return bool 是否需要重命名
     */
    private function shouldRenameDirectory(string $directoryName): bool
    {
        // 如果目录名是纯数字时间戳格式（如：20241112_143025），需要重命名
        if (preg_match('/^\d{8}_\d{6}$/', $directoryName)) {
            return true;
        }

        // 如果目录名是默认的"录音纪要"格式（含时间戳），需要重命名
        if (preg_match('/^.+_\d{8}_\d{6}$/', $directoryName)) {
            return true;
        }
     ...
     * 重命名上传文件所在的目录.
     *
     * @param mixed $fileEntity 文件实体
     * @param string $oldDirectoryPath 旧目录路径
     * @param string $oldDirectoryName 旧目录名称
     ...
     * @param AsrTaskStatusDTO $taskStatus 任务状态
     */
    private function renameUploadedFileDirectory(
        $fileEntity,
        ...
        // 清洗标题并提取时间戳
        $safeTitle = $this->titleGeneratorService->sanitizeTitle($newTitle);
        if (empty($safeTitle)) {
            $this->logger->warning('标题为空，跳过重命名', [
                'task_key' => $taskStatus->taskKey,
            ]);
            return;
        }
        ...
        $parentPath = dirname($oldDirectoryPath);
        $newDirectoryPath = ($parentPath === '.' || empty($parentPath))
            ? $newDirectoryName
            : $parentPath . '/' . $newDirectoryName;

        // 如果新旧路径相同，无需重命名
        if ($newDirectoryPath === $oldDirectoryPath) {
        ...
        try {
            ...
            // 构建完整路径
            $fullOldDirPath = AsrAssembler::buildFileKey($fullPrefix, $workDir, $oldDirectoryPath);
            $fullNewDirPath = AsrAssembler::buildFileKey($fullPrefix, $workDir, $newDirectoryPath);

            // 查找并更新目录实体
            $dirEntity = $this->taskFileDomainService->getByProjectIdAndFileKey((int) $projectId, $fullOldDirPath);
            if ($dirEntity !== null) {
                $dirEntity->setFileName($newDirectoryName);
                $dirEntity->setFileKey($fullNewDirPath);
                $dirEntity->setUpdatedAt(date('Y-m-d H:i:s'));
                $this->taskFileDomainService->updateById($dirEntity);

                // 更新目录下所有文件的路径
                $this->updateChildrenFilePathsForUploadedFile(
                    (int) $projectId,
                    $dirEntity->getFileId(),
                    $fullOldDirPath,
                    $fullNewDirPath,
                ...
                $this->logger->info('上传文件目录重命名成功', [
                    'task_key' => $taskStatus->taskKey,
                    'old_path' => $oldDirectoryPath,
                    'new_path' => $newDirectoryPath,
                    'old_full_path' => $fullOldDirPath,
                    'new_full_path' => $fullNewDirPath,
                ]);
            } else {
                $this->logger->warning('目录实体不存在，跳过重命名', [
                    'directory_path' => $oldDirectoryPath,
                    'task_key' => $taskStatus->taskKey,
                ]);
            }
        } catch (Throwable $e) {
            $this->logger->error('重命名上传文件目录失败', [
                'task_key' => $taskStatus->taskKey,
                'old_path' => $oldDirectoryPath,
                'error' => $e->getMessage(),
            ]);
     ...
     */
    private function updateChildrenFilePathsForUploadedFile(
        int $projectId,
        int $directoryId,
        string $oldDirPath,
        string $newDirPath,
        ...
        try {
            ...
            $this->logger->info('批量更新上传文件子文件路径完成', [
                'task_key' => $taskKey,
            ...
            $this->logger->error('更新上传文件子文件路径失败', [
                'task_key' => $taskKey,
                'error' => $e->getMessage(),
            ]);
        }
