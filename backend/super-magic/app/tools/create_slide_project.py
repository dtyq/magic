...
todo_list 参数应包含完整的 slide-todo.md 文件内容，包含两大部分：**整体设计方案**（全篇统一的视觉规范）和**逐页规划**（每页的内容描述与设计规格）。

⚠️ 模板兼容性（选用模板时强制执行）：
- 整体设计方案中的配色、装饰元素必须从 theme.css 实际定义中提取，不能自行发明
- 每页设计规格中的文字色、强调色必须与 theme.css 的背景色形成高对比度，避免同色系叠加（如模板背景为深棕色，则不能使用棕色文字）
...
## 整体设计方案
- 视觉基调: 基于模板风格的扩展描述（如：科技极简风，以深蓝+荧光青为主色调，整体冷峻专业）
- 配色方案: 主色 bg-slate-900 / 强调色 text-cyan-400 / 背景 bg-gray-950 / 正文 text-gray-300
- 字体层级: 主标题 text-7xl font-black / 区块标题 text-5xl font-bold / 正文 text-2xl / 标签 text-xl
- 统一布局逻辑: 全篇以 60/40 左右分屏为主，封面和过渡页用全屏背景图模式
- 品牌视觉元素: 左侧 4px 渐变竖条 (from-cyan-400 to-blue-600)、右上角三角形几何色块、页码标签统一在右下角
- 基础代码片段参考:
  - 卡片: `class="bg-white/5 border border-white/10 rounded-2xl p-8 backdrop-blur"`
  - 标题区: `class="text-7xl font-black text-white leading-tight tracking-tight"`
  - 强调数字: `class="text-8xl font-black text-cyan-400"`

## 逐页规划

### 第1页 封面（openai-cover.html）

#### 页面内容
- 核心主题: 建立第一印象，点明演讲主题与背景
- 主标题: 「ChatGPT：重塑人类智识的技术革命」
- 副标题: 「OpenAI 发展历程与未来展望」
- 辅助信息: 演讲者姓名、日期（2025年）、机构名称
- 核心数据: 无（封面不需要数据）

#### 设计规格
- 布局方式: 全屏大图背景 + 居中文字叠层，视觉路径从上到下
- 装饰元素: 左下角品牌 Logo 区域、右侧渐变遮罩覆盖 40% 宽度（from-black/80 to-transparent）
- 多媒体需求: 横向全幅背景图，类型：真实摄影图，风格：科技感数据中心或城市夜景
  配图完备度: ✅ 已收集（images/tech-city-night.jpg）
- 静态视觉强调: 主标题区用半透明深色色块衬底（bg-black/50 backdrop-blur-sm）
- 文案字数阈值: 主标题 ≤16字，副标题 ≤20字，无正文段落
- 视觉分层: 视觉中心=主标题；层级：主标题(text-7xl) > 副标题(text-3xl) > 辅助信息(text-xl)

### 第2页 目录（table-of-contents.html）

#### 页面内容
- 核心主题: 展示全篇结构，帮助观众建立阅读预期
- 章节列表:
  01 ChatGPT 诞生背景
  02 技术演进历程
  03 商业化爆发
  04 行业影响分析
  05 未来展望
- 补充说明: 每个章节旁加一句简短描述（≤10字）

#### 设计规格
- 布局方式: 左侧大数字序号 + 右侧章节标题，5行垂直排列，左右各占 20%/80%
- 装饰元素: 每行间隔线（border-b border-white/10）、章节序号与标题对齐
- 多媒体需求: 无图，用几何色块和线条构成视觉锚点
  配图完备度: 不适用
- 静态视觉强调: 序号用强调色 text-cyan-400 text-8xl，章节标题 text-white font-bold
- 文案字数阈值: 章节标题 ≤12字，描述 ≤10字，共5条
- 视觉分层: 视觉中心=序号数字；层级：序号(text-8xl) > 章节标题(text-4xl) > 描述(text-2xl)
...
The todo_list parameter should contain complete slide-todo.md file content. It must include two main sections: **Overall Design Scheme** (unified visual spec for the whole deck) and **Per-Page Planning** (content description + design spec for each page).

⚠️ Template compatibility (mandatory when a template is selected):
- The color palette and decorative elements in the Overall Design Scheme must be extracted from theme.css definitions — never invent colors independently
- Each page's text colors and accent colors must provide sufficient contrast against the background color defined in theme.css — avoid same-hue collisions (e.g., if the template background is dark brown, do not use brown-toned text)
...
- Pages: 10
- Target Audience: Tech professionals, investors
- Key Keywords: ChatGPT, OpenAI, GPT-4, Artificial Intelligence

## Overall Design Scheme
- Visual Tone: Extended description based on template style (e.g., tech-minimal, deep blue + neon cyan palette, cold and professional)
- Color Palette: Primary bg-slate-900 / Accent text-cyan-400 / Background bg-gray-950 / Body text-gray-300
- Typography Scale: Main title text-7xl font-black / Section title text-5xl font-bold / Body text-2xl / Label text-xl
- Unified Layout Logic: 60/40 left-right split as primary layout; full-screen background image for cover and transition pages
- Brand Visual Elements: Left 4px gradient bar (from-cyan-400 to-blue-600), top-right triangle geometric block, page number label consistently in bottom-right
- Base Code Snippets:
  - Card: `class="bg-white/5 border border-white/10 rounded-2xl p-8 backdrop-blur"`
  - Title area: `class="text-7xl font-black text-white leading-tight tracking-tight"`
  - Accent number: `class="text-8xl font-black text-cyan-400"`

## Per-Page Planning

### Page 1 Cover (openai-cover.html)

#### Page Content
- Core Theme: Establish first impression, introduce the presentation topic and background
- Main Title: "ChatGPT: The Technology Revolution Reshaping Human Intelligence"
- Subtitle: "OpenAI Evolution and Future Outlook"
- Supporting Info: Speaker name, date (2025), organization name
- Key Data: None (cover page needs no data)

#### Design Spec
- Layout: Full-screen background image + centered text overlay, visual flow top-to-bottom
- Decorative Elements: Brand logo area at bottom-left, right-side gradient mask covering 40% width (from-black/80 to-transparent)
- Multimedia: Horizontal full-width background image, type: real photography, style: tech-forward data center or city nightscape
  Image Readiness: ✅ Collected (images/tech-city-night.jpg)
- Static Visual Emphasis: Semi-transparent dark color block backing the title area (bg-black/50 backdrop-blur-sm)
- Text Limits: Main title ≤16 chars, subtitle ≤25 chars, no body paragraph
- Visual Hierarchy: Focal point = main title; Levels: main title(text-7xl) > subtitle(text-3xl) > supporting info(text-xl)

### Page 2 Table of Contents (table-of-contents.html)

#### Page Content
- Core Theme: Show full deck structure, help audience build reading expectations
- Chapter List:
  01 ChatGPT Origins
  02 Technical Evolution
  03 Commercial Explosion
  04 Industry Impact Analysis
  05 Future Outlook
- Notes: Add a brief description alongside each chapter (≤8 words)

#### Design Spec
- Layout: Large numeric index on left + chapter title on right, 5 rows vertical, 20%/80% split
- Decorative Elements: Row dividers (border-b border-white/10), chapter number and title aligned
- Multimedia: No image; use geometric color blocks and lines as visual anchors
  Image Readiness: N/A
- Static Visual Emphasis: Index numbers in accent color text-cyan-400 text-8xl, chapter titles text-white font-bold
- Text Limits: Chapter title ≤12 chars, description ≤8 words, 5 entries total
- Visual Hierarchy: Focal point = index number; Levels: index(text-8xl) > chapter title(text-4xl) > description(text-2xl)

### Page 3 ... (filename.html)
...
```
</todo_list_format>
