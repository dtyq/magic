...
import type { MagiClawCreatePayload, MagiClawTemplateOption } from "./MagiClawCreateDialog"

const AVATAR_FILE_ACCEPT = "image/png,image/jpeg,image/jpg,image/webp,image/gif,image/svg+xml"

interface MagiClawCreateSheetProps {
	open: boolean
...
/**
 * MagiClawCreateSheet renders the mobile bottom-sheet create flow for MagiClaw.
 */
export function MagiClawCreateSheet({
	open,
	onOpenChange,
	onCreate,
	isSubmitting = false,
}: MagiClawCreateSheetProps) {
	...
	/** Reset transient form state whenever the sheet closes. */
	...
	return (
		<Sheet open={open} onOpenChange={handleOpenChange}>
			<SheetContent
				...
				data-testid="magi-claw-create-sheet"
			>
				<div className="relative flex max-h-[88dvh] flex-col overflow-hidden bg-background bg-[linear-gradient(155deg,rgba(255,232,220,0.8)_0%,#ffffff_30%)] pb-[max(var(--safe-area-inset-bottom),12px)] dark:bg-card dark:bg-[linear-gradient(155deg,rgba(96,46,18,0.55)_0%,var(--color-card)_38%)]">
					...
					<div className="relative z-10 flex h-14 w-full shrink-0 items-center justify-center px-16 py-2">
						<button
							...
							data-testid="magi-claw-create-sheet-close-button"
						>
						...
						<button
							...
							data-testid="magi-claw-create-sheet-submit-icon-button"
						>
							{isSubmitting ? (
					...
					<input
						...
						className="hidden"
						data-testid="magi-claw-create-sheet-avatar-file-input"
						...
						<div className="relative z-10 flex flex-col gap-[18px] pt-3">
							...
							<div className="flex flex-col gap-2.5">
								<div
									...
									data-testid="magi-claw-create-sheet-template-section"
								>
										...
										{MAGI_CLAW_TEMPLATE_OPTIONS.map((template) => {
											...
											return (
												<button
													...
													data-testid={`magi-claw-create-sheet-template-${template.templateCode}`}
													disabled={isBusy}
													onClick={() =>
								...
								<div
									className="flex flex-col gap-2"
									data-testid="magi-claw-create-sheet-basic-info-section"
								>
									<p className="px-0.5 text-[14px] font-semibold text-foreground">
									...
									<div
										...
										data-testid="magi-claw-create-sheet-basic-info-card"
									>
										<div
											className="flex h-[68px] items-center gap-3 px-[14px]"
											data-testid="magi-claw-create-sheet-avatar-row"
										>
											...
											<div className="flex shrink-0 items-center gap-2.5">
												<div className="relative">
													...
													{customAvatarUrl && !isAvatarUploading ? (
														<Button
															...
															data-testid="magi-claw-create-sheet-remove-avatar-button"
															disabled={isBusy}
															onClick={(event) => {
												...
												<button
													...
													data-testid="magi-claw-create-sheet-upload-button"
													disabled={isBusy}
										...
										<div
											...
											data-testid="magi-claw-create-sheet-name-row"
										>
											<label
											...
											<Input
												...
												data-testid="magi-claw-create-sheet-name-input"
												disabled={isBusy}
												onChange={(event) => {
