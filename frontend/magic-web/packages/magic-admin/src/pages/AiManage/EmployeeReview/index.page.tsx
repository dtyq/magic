...
import { Button, Flex, Input, Modal, message, type TableProps } from "antd"
...
function AIEmployeeReviewPage() {
	const { t } = useTranslation("admin/platform/employeeReview")
	const { t: tCommon } = useTranslation("admin/common")
	const { styles } = useStyles()
	const { AIManageApi } = useApis()
	const hasEditRight = useRights(PERMISSION_KEY_MAP.AGENT_REVIEW_EDIT)
	...
	const { run, loading } = useRequest(
		(arg: ParamsType) => AIManageApi.getOrganizationAgentVersionReviewList(arg),
		{
			manual: true,
			onSuccess: (res) => {
				setData(res.list)
	...
	const { runAsync: reviewAgentVersion, loading: reviewLoading } = useRequest(
		(id: string, data: AiManage.ReviewOrganizationVersionParams) =>
			AIManageApi.reviewOrganizationAgentVersion(id, data),
		{
			manual: true,
		},
	)
	...
	const handleRejectConfirm = useMemoizedFn(async () => {
		...
		try {
			await reviewAgentVersion(currentRejectRecord.id, {
				action: "REJECTED",
	...
	const handleApprove = useMemoizedFn(async (record: DataType) => {
		...
		try {
			await reviewAgentVersion(record.id, {
				action: "APPROVED",
			})
			message.success(tCommon("message.actionSuccess"))
			refreshCurrentList()
...
export default AIEmployeeReviewPage
