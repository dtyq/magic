import localDevConfig from "@admin/apis/config"
import { getCurrentUserRuntimeConfig } from "@admin/stores/user"
...
import { generateSuccessResInterceptor } from "../core/interceptor"
...
export class MagicHttpClient extends HttpClient {
	...
	private setupInterceptors() {
		...
		this.addRequestInterceptor(function request(config) {
			const { language, user, organization } = getCurrentUserRuntimeConfig()
			// 设置通用请求头
...
magicClient.addResponseInterceptor(async (context) => {
	const { request, response } = await generateSuccessResInterceptor()(context)
	return {
		...context,
		request,
		response,
