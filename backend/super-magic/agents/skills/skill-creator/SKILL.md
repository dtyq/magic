...
- 如果同名 skill 在 `.workspace/<workspace-skills-dir>/`（workspace）：询问用户是否覆盖
...
- Same name in `.workspace/<workspace-skills-dir>/` (workspace): Ask user for confirmation.
...
### 执行路径
- **文件工具**（`write_file`、`read_file` 等）的路径相对于 `.workspace/`，直接写 `<workspace-skills-dir>/<skill-name>/...`，**不要带 `.workspace/` 前缀**，否则会写到 `.workspace/.workspace/<workspace-skills-dir>/...`（多嵌套一层）
- `shell_exec` 的**默认工作目录是 `.workspace/`**，命令内部同样**不要带 `.workspace/` 前缀**，直接写 `<workspace-skills-dir>/<skill-name>/...` 即可
- `cwd` 参数是相对 `.workspace/` 解析的，workspace skill 的 scripts 执行时，`cwd` 应为 `<workspace-skills-dir>/<skill-name>`
- skill-creator 自身的脚本执行时，`cwd` 使用 skill-creator 目录的绝对路径（从 `<location>` 取父目录），使用 `python scripts/<script>.py`；skill 路径传绝对路径
...
**Execution paths:**
- **File tools** (`write_file`, `read_file`, etc.) resolve paths relative to `.workspace/`. Use `<workspace-skills-dir>/<skill-name>/...` directly — **never prefix with `.workspace/`**, or files land in `.workspace/.workspace/<workspace-skills-dir>/...` (double-nested).
- `shell_exec` **default working directory is `.workspace/`**. Same rule: inside commands use `<workspace-skills-dir>/<skill-name>/...` without `.workspace/` prefix.
- The `cwd` parameter is resolved relative to `.workspace/`. For workspace skills, use `cwd="<workspace-skills-dir>/<skill-name>"`.
- When running skill-creator's own scripts (e.g., `package_skill.py`, `upload_skill.py`, `aggregate_benchmark.py`), set `cwd` to the **absolute path** of the skill-creator directory (derived from the skill's `<location>` tag); use `python scripts/<script>.py`; pass skill paths as absolute paths.
