...
High-level workflow: Capture Intent → Interview → Plan → Write → Test → Iterate → Optimize → Package.
...
访谈完成后，**先**输出一份规划文档供用户确认，再开始写 SKILL.md。
...
保存到 `.workspace/skills/<skill-name>/plan.md`，等用户确认后再执行创建。
-->
### Phase 3: Output Plan Document

After the interview, **first** output a plan document for user confirmation before writing SKILL.md.
...
Save to `.workspace/skills/<skill-name>/plan.md`. Wait for user confirmation before proceeding.

---

<!--zh
### 阶段四：检查名称冲突并编写 SKILL.md（Write）

**创建前必须先检查冲突：**
-->
### Phase 4: Check Conflicts and Write SKILL.md
...
<!--zh
**冲突处理规则：**
- 如果同名 skill 在 `agents/skills/`（内置）：**不可覆盖**，必须告知用户换一个名称
...
-->
**Conflict rules:**
- Same name in `agents/skills/` (built-in): **Cannot override**. Tell the user to choose a different name.
- Same name in `.workspace/skills/` (workspace): Ask user for confirmation.
  ...
  - This ensures the new skill starts clean
...
├── SKILL.md          (required)
├── plan.md           (planning document from Phase 3)
├── evals/
...
测试用例保存到 `.workspace/skills/<skill-name>/evals/evals.json`：
-->
### Phase 5: Test

Write 2-3 realistic test prompts — the kind a real user would actually say. Share with the user first.

Save test cases to `.workspace/skills/<skill-name>/evals/evals.json`:
...
# Packaged file: .workspace/skills/<skill-name>/<skill-name>.skill
```
...
### 执行路径
- `shell_exec` 的**默认工作目录是 `.workspace/`**，所以在命令内部写相对路径时**不要带 `.workspace/` 前缀**，直接写 `skills/<skill-name>/...` 即可，否则会多一层变成 `.workspace/.workspace/skills/...`
...
**Execution paths:**
- `shell_exec` **default working directory is `.workspace/`**. Inside commands, use paths **without the `.workspace/` prefix** — write `skills/<skill-name>/...` directly. Including `.workspace/` would produce `.workspace/.workspace/skills/...` (double-nested).
